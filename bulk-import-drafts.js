...
function refreshDraftButton(){
  ...
  if(source){
    ...
    btn.title='Load the previously saved bulk-import draft.';
  }
}
...
async function saveDraft(){
  if(!authed)return;
  const source=$('source')?.value?.trim()||'';
  if(!source){refreshDraftButton();status('There are no questions to save. Use Load Draft to restore the previous draft.',true);return;}
  const btn=$('draftButton');btn.disabled=true;btn.textContent='Saving…';
  try{
    await setDoc(draftRef(),snapshotForm());
    hasDraft=true;
    btn.textContent='Save Draft';
    status('✓ Current data saved as the draft. The previous draft was replaced.');
  }catch(e){
    console.error(e);
    status('Could not save draft. Check Firestore permissions.',true);
  }finally{btn.disabled=false;refreshDraftButton();}
}
...
async function loadDraft(){
  ...
  try{
    ...
    if(!snap.exists()){
      hasDraft=false;
      btn.textContent='Load Draft';
      status('No saved draft found.',true);
      return;
    }
    const d=snap.data();
    ...
    console.error(e);
    status('Could not load draft. Check Firestore permissions.',true);
    refreshDraftButton();
  }finally{btn.disabled=false;refreshDraftButton();}
}

async function removeDraft(){try{await deleteDoc(draftRef());hasDraft=false;refreshDraftButton();}catch(e){console.warn('Draft cleanup failed:',e);}}
...
function installButton(){
  ...
  if(btn.dataset.draftListener!=='1'){
    ...
    btn.addEventListener('click',()=>{
      const source=$('source')?.value?.trim()||'';
      if(source)saveDraft();else loadDraft();
    });
  }
  const source=$('source');
  if(source&&source.dataset.draftInputListener!=='1'){
    source.dataset.draftInputListener='1';
    source.addEventListener('input',refreshDraftButton);
  }
  refreshDraftButton();
}
