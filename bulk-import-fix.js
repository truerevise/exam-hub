...
function ensureRequiredTag(){
  const el=$('tags');if(!el)return;
  const tags=el.value.split(',').map(x=>x.trim()).filter(Boolean);
  if(!tags.some(x=>x.toLowerCase()==='set economics'||x.toLowerCase()==='set-economics')) tags.push('SET Economics');
  el.value=tags.join(', ');
}
...
function isSequenceValid(items){if(!items.length)return false;const s=sequenceIssues(items);return !s.duplicates.length&&!s.missing.length&&!s.outOfRange.length;}

function renderPreview(parsed){
  const seq=sequenceIssues(parsed),valid=parsed.filter(q=>!q.errors.length).length,max=parsed.length?Math.max(...parsed.map(q=>q.num)):0,ok=isSequenceValid(parsed);
  ...
  const warnings=[];if(seq.duplicates.length)warnings.push(`duplicate numbers: ${seq.duplicates.join(', ')}`);if(seq.missing.length)warnings.push(`missing numbers: ${seq.missing.slice(0,30).join(', ')}${seq.missing.length>30?'…':''}`);if(seq.outOfRange.length)warnings.push(`out-of-range numbers: ${seq.outOfRange.join(', ')}`);
...
async function importQuestions(parsed){
  const valid=parsed.filter(q=>!q.errors.length);if(valid.length!==parsed.length||!isSequenceValid(parsed)){$('status').className='status error';$('status').textContent='Fix validation/sequence errors before importing.';return;}
  const exam=$('exam').value.trim(),subject=$('subject').value.trim(),previousExam=$('previousExam').value.trim();ensureRequiredTag();
  const globalTags=$('tags').value.split(',').map(x=>x.trim().toLowerCase()).filter(Boolean),tags=[...new Set([...globalTags,REQUIRED_TAG])],marks=Number($('marks').value)||0,negativeMarks=Number($('negative').value)||0;
  ...
  try{
    for(let i=0;i<valid.length;i+=400){const batch=writeBatch(db);if(i===0){batch.set(doc(db,'exams','exam-'+String(exam).trim().toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,'').slice(0,100)),{name:exam,title:exam,subject,updatedAt:serverTimestamp(),createdBy:auth.currentUser?.email||''},{merge:true});}
      for(const q of valid.slice(i,i+400)){const ref=doc(collection(db,'questions'));batch.set(ref,{exam,subject,previousExam,tags:[...new Set([...(q.tags.length?q.tags:tags),REQUIRED_TAG])],marks,negativeMarks,questionNumber:q.num,question:q.question,questionTe:q.questionTe||'',questionImageUrl:q.imageUrls?.question||'',options:q.options,optionsTe:q.optionsTe,optionImageUrls:{A:q.imageUrls?.A||'',B:q.imageUrls?.B||'',C:q.imageUrls?.C||'',D:q.imageUrls?.D||''},correctAnswer:q.correctAnswer,answerType:['A','B','C','D'].includes(q.correctAnswer)?'single':q.correctAnswer==='ALL'?'all':q.correctAnswer==='MULTIPLE'?'multiple':'deleted_or_all_awarded',explanation:q.explanation,explanationTe:q.explanationTe||'',hasTelugu:q.hasTelugu===true,availableLanguages:q.hasTelugu?['en','te']:['en'],answerSource:'bulk-import',createdAt:serverTimestamp()});}
      await batch.commit();}
    const slug=String(subject).trim().toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,'').slice(0,100);await setDoc(doc(db,'subjects','subject-'+slug),{name:subject,updatedAt:serverTimestamp(),createdBy:auth.currentUser?.email||''},{merge:true});
    $('status').className='status';$('status').textContent=`✓ Successfully imported all ${valid.length} questions into ${exam} → ${subject}.`;btn.textContent='Imported';
...
function initFix(){
  ensureRequiredTag();
  const tagInput=$('tags');if(tagInput)tagInput.addEventListener('change',ensureRequiredTag);
  const preview=$('preview'),importBtn=$('importBtn');if(!preview||!importBtn)return;let parsed=[];
  preview.onclick=()=>{parsed=parseQuestions($('source').value);window.__bulkFixedParsed=parsed;renderPreview(parsed);const invalid=parsed.filter(q=>q.errors.length).length,ok=isSequenceValid(parsed);$('status').className='status'+(invalid||!ok?' error':'');$('status').textContent=!parsed.length?'No questions found.':ok&&!invalid?`✓ Found ${parsed.length} questions (Q1–Q${Math.max(...parsed.map(q=>q.num))}). They are ready to import.`:`Found ${parsed.length} questions. ${invalid} need correction; check the Q-number sequence.`;};
