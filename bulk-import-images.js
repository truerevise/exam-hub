...
function friendlyStorageError(error,part){
  const code=String(error?.code||'').toLowerCase();
  if(code.includes('unauthorized')||code.includes('permission-denied')) return `Firebase Storage permission denied for ${part}. Deploy the Storage rules from storage.rules and make sure the admin account is signed in.`;
  if(code.includes('unauthenticated')) return `Firebase Storage requires admin sign-in for ${part}. Please sign in again.`;
  if(code.includes('canceled')||code.includes('cancelled')) return `${part} upload was cancelled.`;
  if(code.includes('quota')) return `Firebase Storage quota/limit was reached while uploading ${part}.`;
  if(code.includes('retry-limit')) return `${part} upload could not complete because the network connection was interrupted.`;
  if(code.includes('invalid-checksum')) return `${part} upload failed integrity validation. Please choose the image again.`;
  if(code.includes('unknown')) return `Firebase Storage could not complete the ${part} upload. Check Storage is enabled for the Firebase project.`;
  return `${part} upload failed: ${error?.message||'unknown Firebase Storage error'}`;
}
...
function addImageFields(){
  ...
  fields.forEach(([id,label,help])=>{
    const div=document.createElement('div'); div.className='field image-field';
    div.innerHTML=`<label>🖼️ ${label}</label><input id="${id}" type="file" accept="image/*"><div class="help">${help} Maximum 5 MB.</div>`;
...
async function nextQuestionNumber(exam,subject){
  ...
  snap.docs.forEach(d=>{
    const q=d.data();
    if(String(q.exam||'').trim()===exam&&String(q.subject||'').trim()===subject){
...
async function uploadImage(file,exam,subject,questionNumber,part,onProgress){
  if(!file) return '';
  if(!file.type.startsWith('image/')) throw new Error(`${part} must be an image file.`);
  if(file.size>MAX_IMAGE_SIZE) throw new Error(`${part} is larger than 5 MB.`);

  const ext=(file.name.split('.').pop()||'jpg').toLowerCase().replace(/[^a-z0-9]/g,'')||'jpg';
  const safePart=part.toLowerCase().replace(/[^a-z0-9]+/g,'-');
  const path=`question-images/${slug(exam)}/${slug(subject)}/q${questionNumber}-${safePart}-${Date.now()}-${Math.random().toString(36).slice(2,8)}.${ext}`;
  ...
  return await new Promise((resolve,reject)=>{
    ...
    const timer=setTimeout(()=>{
      ...
      finish(reject,new Error(`${part} upload timed out after 60 seconds. Check your internet connection and Firebase Storage.`));
    },UPLOAD_TIMEOUT_MS);
...
async function addWithImages(){
  ...
  btn.disabled=true;
  btn.textContent=selected.length?'Uploading…':'Adding…';
  status('');

  try{
    const questionNumber=await nextQuestionNumber(exam,subject);
    const uploaded={question:'',A:'',B:'',C:'',D:''};
    const labels={question:'Question image',A:'Option A image',B:'Option B image',C:'Option C image',D:'Option D image'};

    for(const [key,file] of Object.entries(files)){
      if(!file)continue;
      status(`Uploading ${labels[key]} (0%)…`);
      uploaded[key]=await uploadImage(file,exam,subject,questionNumber,key,(percent)=>status(`Uploading ${labels[key]} (${percent}%)…`));
    }

    status('Saving question…');
    const questionRef=doc(collection(db,'questions'));
    await setDoc(questionRef,{
      exam,subject,previousExam,tags,marks,negativeMarks,questionNumber,question,
      questionImageUrl:uploaded.question,
      options:{A,B,C,D},
      optionImageUrls:{A:uploaded.A,B:uploaded.B,C:uploaded.C,D:uploaded.D},
      correctAnswer,answerType:'single',explanation,answerSource:'individual-bulk-page',createdAt:serverTimestamp()
    });
    ...
    console.error('Individual question/image upload failed:',e);
    const failedPart=selected.length?'Image upload':'Question save';
    status(e?.code?friendlyStorageError(e,failedPart):String(e?.message||'Could not add question. Please try again.'),true);
  }finally{
    btn.disabled=false;
    btn.textContent='Add Question to Series';
  }
}
...
// Capture phase prevents the older handler in bulk-import.js from running.
